...
public class SECTR_IntroWindow : SECTR_Window 
{
	...
	static string prefName = "SECTR_Intro_Show";

	bool hasComplete = false;
	bool didInit = false;

	string infoURL = IntroData.siteFileBaseURL + "sectr_info.xml";
	WWW infoWWW;
	...
	public SECTR_IntroWindow()
	{
		infoWWW = new WWW(infoURL);
	...
	void Update()
	{
		if(infoWWW != null && infoWWW.isDone)
		{
	...
	{
		if(EditorPrefs.GetBool(prefName))
		{
	...
	private void _DrawOptOut(Rect mainRect)
	{
		...
		GUI.enabled = true;
		bool playerPref = EditorPrefs.GetBool(prefName, true);
...
#endif
		EditorPrefs.SetBool(prefName, playerPref);
	}
