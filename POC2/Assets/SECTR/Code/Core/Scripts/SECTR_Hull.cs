...
public abstract class SECTR_Hull : MonoBehaviour
{
	...
	/// Returns the verts in counterclockwise order.
	public Vector3[] VertsCCW
	{
		get { ComputeVerts(); return vertsCCW; } 
	}
	...
	protected void ComputeVerts()
	{
		if(HullMesh != previousMesh)
		{
			if(HullMesh)
			{
				...
				{
					return SECTR_Geometry.CompareVectorsCW(a, b, meshCentroid, meshNormal);
				});
				...
				// Build both the final CW and CCW lists
				vertsCCW = vertsCW;
				Array.Reverse(SECTR_Geometry.CompareVectorsCW(vertsCW[0], vertsCW[0], meshCentroid, meshNormal) >= 0 ? vertsCCW : vertsCW);
			}
			else
			{
				meshNormal = Vector3.zero;
