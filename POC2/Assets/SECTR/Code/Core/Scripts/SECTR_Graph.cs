		...
		for(int initialSectorIndex = 0; initialSectorIndex < numInitialSectors; ++initialSectorIndex)
		{
			...
			for(int portalIndex = 0; portalIndex < numPortals; ++portalIndex)
			{
				...
				if((portal.Flags & stopFlags) == 0)
				{
					...
					newElement.Cost = Vector3.SqrMagnitude(start - portal.transform.position);
					float estimate = Vector3.SqrMagnitude(goal - portal.transform.position);
		...
		while(openSet.Count > 0)
		{
			...
			for(int portalIndex = 0; portalIndex < numPortals; ++portalIndex)
			{
				...
				if(portal != current.Portal && (portal.Flags & stopFlags) == 0)
				{
					...
					neighborElement.Cost = current.Cost + Vector3.SqrMagnitude(neighborElement.Portal.transform.position - current.Portal.transform.position);
					float estimate = Vector3.SqrMagnitude(goal - neighborElement.Portal.transform.position);
