...
public class MinesPuzzleState : SimpleFollowLucyState
{
    ...
    {
        if (draggingBack)
            UpdateDragPosition(script.Player);
    ...
    {
        script.Player.transform.position = DragBackLocation.transform.position;
        script.Player.transform.rotation = DragBackLocation.transform.rotation;
    ...
    public override void PlayerEnteredTrigger(UnityEngine.Collider collider, Story script)
    {
        ...
        if (collisionType != MineCollisionType.None) // player collided with a mine
        {
            ...
            {
                transportingBackSoundPlayer = AudioManager.PlayAudio(new AudioObject(script.Player, DragBackSound, 1, 0));
                script.SetPlayerMovementLocked(true);
                MineField.RemoveMine(collidedMine);
                draggingBack = true;
                draggingBackTime = 0f;
                dragBackStartPosition = new PositionRotation(script.Player.transform.position, script.Player.transform.rotation);
