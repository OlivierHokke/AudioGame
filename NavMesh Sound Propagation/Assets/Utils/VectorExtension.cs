...
{
    public static Vector3 Min(this Vector3 me, Vector3 other)
    {
        return new Vector3(Mathf.Min(me.x, other.x), Mathf.Min(me.y, other.y), Mathf.Min(me.z, other.z));
    }
    public static Vector3 Max(this Vector3 me, Vector3 other)
    {
        return new Vector3(Mathf.Max(me.x, other.x), Mathf.Max(me.y, other.y), Mathf.Max(me.z, other.z));
    }
    public static bool Inside(this Vector3 me, Vector3 min, Vector3 max)
    {
        return min.x <= me.x && me.x <= max.x
            && min.y <= me.y && me.y <= max.y
            && min.z <= me.z && me.z <= max.z;
    }
    // ------------------------------------------
    ...
    public static Vector3 mutate(this Vector3 v, float amount)
    {
        return new Vector3(v.x + Randomg.Symmetrical(amount), v.y + Randomg.Symmetrical(amount), v.z + Randomg.Symmetrical(amount));
    }
